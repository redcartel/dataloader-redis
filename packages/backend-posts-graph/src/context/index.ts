...
export function contextFactory(redisConnection : RedisClientType, postgresConnection: Pool) {
    return (context) => {
        const account = JSON.parse(context.request.headers.get('account') ?? '{}')
        return {account, loaders: new PostsLoaders(redisConnection, postgresConnection), accountLoaders: new AccountsLoaders(redisConnection, postgresConnection)}
    };
}
