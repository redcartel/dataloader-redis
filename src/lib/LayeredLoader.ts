...
export class LayeredLoader<K, V> extends DataLoader<K, V> {
    ...
    constructor(layers: Layer<K, V>[], options?: Options<K, V> & { noDeduplication?: boolean, waitForCacheWrite?: boolean }) {
            ...
            async function readWriteLayer(idx: number, keys: readonly K[]): Promise<(V | Error)[]> {
                ...
                if (idx < len - 1) {
                    ...
                    if (_badKeys.length > 0) {
                        ...
                        const _updateMap = new Map(_badKeys.map((key, idx) => [key, _nextVals[idx]]));
    ...
    public async asyncClear(key: K) {
        super.clear(key);
        await Promise.all(this.layers.map(layer => layer.clear && layer.clear(key)));
    }
    ...
    override clearAll() {
        console.warn('outer dataloader cache cleared, intermediate layers not cleared');
        super.clearAll();
        return this;
    }
